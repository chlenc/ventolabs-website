...
export const en: Dictionary = {
  ...
  case_pages: {
    ...
    "erp-agent": {
      title: "1C Agent",
      kicker: "AI-ops · Internal speed",
      navLabel: "1C Agent",
      heroTitle: "Work with 1C in the language of business — without risking the core",
      heroDescription: "The agent runs on Claude: developers prepare changes faster via prompts, plans and extensions, while managers read and update data only within their allowed scope. Separated roles, review flows, audit log throughout.",
      cardSummary: "Permission-based AI layer over 1C: acceleration for developers, safe actions for managers.",
      problems: [
      ...
      guide: {
        empathy: "We don't let AI change the core freely, we don't blur manager and developer roles, and we don't sell a magic button.",
        authority: "We build controlled acceleration over 1C: a permissioned AI layer where every action passes through roles, approval and an audit log.",
      },
      plan: [
        { title: "Map roles and limits", description: "We describe roles, accessible objects and hard limits. We decide what managers can do and what only developers can do." },
        { title: "Connect 1C", description: "We connect 1C via OData, HTTP and web-services and expose only the actions each user type actually needs." },
        { title: "Launch the flows", description: "Developers: prompt → plan → extension → review → apply. Managers: prompt → allowed action → journal → control." },
      ],
      included: [
      ...
      stakes: "If the business needs speed and IT needs safety, you don't need another chatbot. You need a managed AI layer over 1C.",
      faq: [
        { q: "Can managers actually change 1C data through AI?", a: "Yes — but only within their role. If a sales manager can create a client and an order, the agent lets them do it via prompt. If they can't, the agent won't offer the action. This is AI through permissions, not AI instead of them." },
        { q: "How is the core configuration protected from AI?", a: "AI never writes to the base configuration. All changes go through configuration extensions, plus plan review and developer approval before apply. The standard configuration stays intact and upgradeable." },
        { q: "What if the AI makes a mistake preparing a change?", a: "That's why the dev flow is prompt → plan → extension → review → apply. The developer sees the plan and the diff before apply. Nothing lands in prod without explicit approval. This is the Claude Code pattern — accelerator, not autopilot." },
        { q: "Which 1C versions do you work with?", a: "1C:Enterprise 8.3+, standard configurations (Accounting, Trade, KA, ERP) and custom ones. Integration via OData, HTTP services and web-services." },
        { q: "Where are prompts and context stored?", a: "In your perimeter — your server or your cloud. Claude models are called via API on business tier, where data isn't used for training. For strict requirements an on-prem model is possible." },
      ],
      seo: {
        title: "1C Agent — Permissioned AI layer for 1C on Claude",
        description: "Claude-powered AI accelerates 1C developers via extensions and reviews, while managers get safe actions on data strictly within their role.",
      },
    },
  },
